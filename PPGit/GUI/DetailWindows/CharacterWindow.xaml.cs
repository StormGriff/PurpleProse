...
namespace PPGit.GUI.DetailWindows
{
    ...
    public partial class CharacterWindow : /*Adornment_Win//*/MetroWindow
    {
            ...
            if (NameBox.Text != "")
            {
                try
                {
                    ...
                    if (Directory.Exists(mainLists.projectDir + "\\items\\characters\\" + Person.Name.ToLower().Replace(" ", string.Empty) + Person.Number))
                    {
                        ...
                        string newDir = mainLists.locationToSaveTo;
                        //rename folder and deletes the old one
                        Directory.Move(Person.Directory, newDir);
                    }
                    else Directory.CreateDirectory(mainLists.locationToSaveTo);

                    //if the info file already exists, rename the character's info file to the new name
                    if (File.Exists(mainLists.locationToSaveTo + "\\" + TextOps.ToDirectorySafe(Person.Name) + ".info"))
                    {
                        string oldFile = mainLists.locationToSaveTo + "\\" + TextOps.ToDirectorySafe(Person.Name) + ".info";
                        string newFile = mainLists.locationToSaveTo + "\\" + TextOps.ToDirectorySafe(NameBox.Text) + ".info";
                        //rename info file and deletes old one
                        File.Move(oldFile, newFile);
                    }
                    else
                    {
                        File.Create(mainLists.locationToSaveTo + "\\" + TextOps.ToDirectorySafe(NameBox.Text) + ".info");
                    }
        ...
        private void AgeBox_LostKeyFocus(object sender, KeyboardFocusChangedEventArgs e)
        {
            if (AgeBox.Text != "")
            {
                try { Person.charAge = Convert.ToInt64(AgeBox.Text) }// I figured sometimes the ages of fictional characters can get long.
        ...
        private void Add_picture(string FQP)
        {
            ...
            #region More commented out code.
            //try
            //{
            //    double yval = 0;
        ...
        {
            mainLists.locationToSaveTo = mainLists.projectDir + "\\items\\characters\\" + TextOps.ToDirectorySafe(NameBox.Text) + Person.Number;// + @"\texts";

            string fileName = "desc";
            string fileExt = ".txt";
            string FQPath = mainLists.locationToSaveTo + "\\texts\\" + fileName + fileExt;//Fully Qualified Path
            bool isEmpty = true;

            if (Directory.Exists(mainLists.locationToSaveTo))
            {
                isEmpty = !Directory.EnumerateFiles(mainLists.locationToSaveTo + "\\texts").Any()
            }

            else
            {    
                Directory.CreateDirectory(mainLists.locationToSaveTo);
                Directory.CreateDirectory(mainLists.locationToSaveTo + "\\images");
                Directory.CreateDirectory(mainLists.locationToSaveTo + "\\texts");
            }
            ...
            if(isEmpty)
            {
                ...
                mainLists.fullEditor = new TextEditor.TextEditor(FQPath, fileName + fileExt, false);
            }
            else
            {
                mainLists.fullEditor = new TextEditor.TextEditor();
                mainLists.fullEditor.OpenTextFile(mainLists.locationToSaveTo + "\\texts\\", fileName + fileExt);
            }
